...
- SEO change outcomes are descriptive pre/post evidence, not causal claims. Keep GSC and aggregate business windows comparable; missing business evidence is `insufficient_data`, not zero.
...
- Technical-rule effects require at least six verified fixes with complete 14-day pre/post GSC history, exclude comparisons crossing a GSC measurement regime, and apply FDR across tested rules. Call them associations, never causal impact.
...
- Technical issues are verified only by a complete later audit with the same config fingerprint. Partial or incomparable runs cannot prove absence.
